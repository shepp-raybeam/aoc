...
class Solution():
    ...
    def process_input(self):
        ...
        state = np.full((size+2, size+2, size+2, size+2), '.')

        state[int(size/2)+1, int(size/2)+1, 1:-1, 1:-1] = cube2d
        return state
    ...
    def cycle(self):
        ...
        for i in flips:
            x,y,z,w = i
            current = self.state[x,y,z,w]
            self.state[x, y, z, w] = '#' if current == '.' else '.'

    def add_border(self): #better if it only added when previous plane is not all .
        size = self.state.shape[0]
        newstate = np.full((size+2, size+2, size+2, size+2), '.')
        newstate[1:self.state.shape[0]+1, 1:self.state.shape[1]+1, 1:self.state.shape[2]+1, 1:self.state.shape[3]+1] = self.state 
        self.state = newstate
